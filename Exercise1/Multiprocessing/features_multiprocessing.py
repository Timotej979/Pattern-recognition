from shapes_multiprocessing import *
import cv2
...
def oversampleSignal(signal, newN = 64):
...
def calculateFastFourierCoeficients(inputImage, kmax, lmax):
    """Calculate a vector of features from the longest shape on the picture"""
    binaryImage = preprocessImage(inputImage)

    shapeSearcher = Searcher(binaryImage)
    shapeSearcher.searchShapes()
    longestShape = shapeSearcher.getLongestShape()

    signal = shape2Signal(longestShape)
    signal = oversampleSignal(signal)
