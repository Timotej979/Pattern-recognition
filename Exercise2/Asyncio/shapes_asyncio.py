...
class Searcher():
    ...
    async def checkVicinity(self, x_z, y_z):
    ...
    async def shapeFollower(self, x_z, y_z):
        """Follow the shape with starting point (x_z, y_z)"""
        x_o = x_z
        y_o = y_z

        check = await self.checkVicinity(x_z, y_z)
        if check == False:
            return 0
        ...
        while True:
            exit = await self.move(x_o, y_o, enter)
            await self.setPixelNote(enter, exit, x_o, y_o)
    ...
    async def view(self, x_o, y_o, direction):
    ...
    async def move(self, x_o, y_o, enter):
        ...
        for direction in priority:
            if await self.view(x_o, y_o, direction):
                return direction

    async def setPixelNote(self, enter, exit, x_o, y_o):
    ...
    async def getLongestShape(self):
    ...
    async def drawAllShapes(self, grayScaleInterval):
    ...
    async def drawShape(self, shape, grayColor):
